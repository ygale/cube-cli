...
def test_print_cube_solved_top_color() -> None:
  ...
  cube: Cube = solved(initial=_ref)
  top_char: str = side_color(cube, Side.TOP).name[0].lower()
  lines: list[str] = list(print_cube(cube))
  row: str; ch: str
...
def test_print_cube_solved_bottom_color() -> None:
  ...
  cube: Cube = solved(initial=_ref)
  bot_char: str = side_color(cube, Side.BOTTOM).name[0].lower()
  lines: list[str] = list(print_cube(cube))
  row: str; ch: str
...
def check_print_output(
  ...
  '''Check that the sequence of colors in each output line matches.

  Returns True if the color characters extracted from each line of
  actual match the corresponding entry in expected.
  '''
...
def test_print_after_cw_move(side: Side) -> None:
  '''Color order in print output matches expected after a CW move.'''
  cube: Cube = solved(initial=_ref)
